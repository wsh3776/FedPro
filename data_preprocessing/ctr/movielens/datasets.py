...
# TODO: 2. user_id和item_id作embedding
...
def get_negative_samples_per_user_3(ratio_of_pos_to_neg=1):
    """
    用于生成负样本 numpy a little fast
    Args:
        ratio_of_pos_to_neg: 正负样本的比例
    ...
    for user_id in tqdm(range(1, len(users) + 1), desc="Generate negative items"):
        ...
        # 负样本的数量不能超过可选的数量
        num_negative_items = min(len(positive_items) * ratio_of_pos_to_neg, len(items) - num_positive_items)
...
def get_negative_samples_per_user(ratio_of_pos_to_neg=1):
    """
    用于生成负样本 numpy a little fast
    Args:
        ratio_of_pos_to_neg: 正负样本的比例
...
def get_negative_samples_per_user_2(ratio_of_pos_to_neg=1):
    """
    ...
    for user_id in tqdm(range(1, len(users) + 1), desc="Generate negative samples for each user"):
        items = list(movies['movie_id'])  # Note: movie_id 不是连续的，中间有缺失, 总共3883部，但movie_id最大3952
        positive_items = list(ratings[ratings.user_id == user_id]['movie_id'])
        num_negative_items = len(positive_items) * ratio_of_pos_to_neg
